...
const HeroSection = () => {
  ...
  return (
    <section className="relative w-full h-60 sm:h-[500px] md:h-[600px] overflow-hidden md:rounded-tl-2xl">
      <div
      ...
      >
        {extendedSlides.map((slide, index) => (
          <div key={index} className="min-w-full shrink-0 relative h-full md:rounded-tl-2xl">
            <Image
              src={slide}
              alt={`Slide ${index + 1}`}
              fill
              priority={index === 0}
              className="object-fill md:rounded-tl-2xl"
            />
          </div>
        ))}
      </div>
